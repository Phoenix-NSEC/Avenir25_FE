  ...
  return (
    <group>
      {Array.from({ length: 15 }).map((_, i) => (
        <SmokePlane
          key={i}
          position={[(Math.random() - 0.5) * 10, (Math.random() - 0.5) * 3, (Math.random() - 0.5) * 5 - 5]}
...
const AnimatedText: React.FC = () => {
  ...
  useFrame((state) => {
    if (text.current && visible) {
      (text.current.material as THREE.MeshStandardMaterial).opacity = Math.min(1, state.clock.elapsedTime * 0.2);
      (text.current.material as THREE.MeshStandardMaterial).emissiveIntensity = Math.min(
        0.5,
...
const MagicSparkles: React.FC = () => {
  return <Sparkles count={150} scale={10} size={2} speed={0.3} color="#7e22ce" opacity={0.4} />;
};
    ...
    <div className="h-screen w-full bg-gradient-to-b from-gray-950 to-black relative overflow-hidden">
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center z-50 bg-black">
          <div className="text-purple-600 text-xl">Summoning mystical forces...</div>
        </div>
      )}
