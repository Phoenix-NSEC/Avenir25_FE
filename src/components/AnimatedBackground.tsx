...
const AnimatedBackground = () => {
  ...
  class MagicElement {
    ...
    constructor(x: number, y: number) {
      ...
      const types: Array<'star' | 'polygon' | 'ring' | 'comet'> = ['star', 'polygon', 'ring', 'comet'];
    ...
    draw(ctx: CanvasRenderingContext2D, time: number) {
      ...
      switch(this.type) {
        case 'star':
          this.drawStar(ctx, 0, 0, this.points, this.size * 0.5, this.size, alpha);
          break;
        case 'polygon':
    ...
    drawStar(ctx: CanvasRenderingContext2D, cx: number, cy: number, spikes: number, innerRadius: number, outerRadius: number, alpha: number) {
      let rot = Math.PI / 2 * 3;
      let x = cx;
      let y = cy;
    ...
    drawPolygon(ctx: CanvasRenderingContext2D, cx: number, cy: number, sides: number, radius: number, alpha: number) {
      ctx.beginPath();
    ...
    drawRing(ctx: CanvasRenderingContext2D, cx: number, cy: number, outerRadius: number, thickness: number, alpha: number, time: number) {
    ...
    drawComet(ctx: CanvasRenderingContext2D, cx: number, cy: number, radius: number, alpha: number) {
      // Comet head
      ...
      const tailGradient = ctx.createLinearGradient(
        cx, cy,
        cx - this.speedX * radius * 3, cy - this.speedY * radius * 3
      );
      tailGradient.addColorStop(0, this.color.replace('0.7', (alpha * 0.8).toString()));
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      ...
      const particleCount = Math.floor((windowSize.width * windowSize.height) / 12000);
      ...
      const magicCount = Math.floor((windowSize.width * windowSize.height) / 150000) + 2;
  ...
  const orbs = Array(6).fill(0).map((_, i) => {
    const orb = {
      x: 10 + (i * 20),
      y: 20 + (i * 10),
  ...
  return (
    ...
    >
      ...
      {/* Light beam effect */}
      <div className="absolute top-0 left-1/4 w-32 h-screen bg-gradient-to-b from-purple-500/5 via-transparent to-transparent transform -rotate-45 blur-xl animate-pulse" 
        style={{ animationDuration: '8s' }}
      />
      <div className="absolute top-0 right-1/3 w-24 h-screen bg-gradient-to-b from-cyan-500/5 via-transparent to-transparent transform rotate-45 blur-xl animate-pulse" 
