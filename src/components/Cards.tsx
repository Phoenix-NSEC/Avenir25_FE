...
export default function Cards() {
  ...
  return (
    <section id="events" className="py-20 px-4 sm:px-6 relative overflow-hidden">
      {/* Background elements */}
      <div className="absolute top-0 left-0 w-full h-full">
        <div
          className="absolute top-1/4 right-1/4 w-64 h-64 rounded-full bg-purple-500/10 blur-3xl"
          style={{
            transform: isMobile ? "" : `translate(${mousePosition.x * -20}px, ${mousePosition.y * -20}px)`,
          }}
        ></div>
        <div
          className="absolute bottom-1/3 left-1/3 w-96 h-96 rounded-full bg-cyan-500/10 blur-3xl"
          style={{
            transform: isMobile ? "" : `translate(${mousePosition.x * 30}px, ${mousePosition.y * 30}px)`,
          }}
        ></div>
      </div>
